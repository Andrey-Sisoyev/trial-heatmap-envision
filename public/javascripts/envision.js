...
Graph.prototype = {
  ...
  getDataY_forDataX: function(dataX, serieIdx) {
    ...
    xFrom = allData[iFrom][0];
    yFrom = allData[iFrom][1];

    iTo = Math.min(iFrom + 1, len);
...
Flotr.addPlugin('hit', {
  ...
  hit : function (mouse) {
    ...
    if (!prevHit || prevHit.index !== n.index || prevHit.seriesIndex !== n.seriesIndex || (n.freeHit && prevHit.x !== n.x)) {
      this.hit.clearHit();
      if (n.series && n.mouse && n.mouse.track) {
        this.hit.drawMouseTrack(n); // draw text-box
        this.hit.drawHit(n); // #4772 for linecharts
